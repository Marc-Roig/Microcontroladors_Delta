...
static final int SERIAL_BUFFER_LEN = 5;
static final int SERIAL_COMMAND_MAX_LEN = 16;
...
public class Buffer {
  ...
  Buffer() {
    command = new String[SERIAL_COMMAND_MAX_LEN];
    start = 0;
    end = 0;
    full = false;
    empty = true;
    
    for (int i = 0; i < SERIAL_COMMAND_MAX_LEN; i++) {
      command[i] = "";
    }
  }
...
void parse_command(String command) {
  ...
  switch (command_num) {
    case SEND_ANGLES:
      ...
      break;
    case END_OF_STREAM:
      myPort.write("G06" + "\n"); //WAIT TO RECIEVE MORE INSTRUCTIONS
      println("Sending G06");
      break;
    default:
      break;
